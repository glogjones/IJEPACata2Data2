...
def make_fits_cutout_dataset(
    ...
    # Display the first cutout in every batch
    logger.info("Displaying the first cutout in every batch:")
    for batch_idx, batch in enumerate(data_loader):
        cutout = batch[0].numpy().squeeze()  # Get the first cutout in the batch
        plt.imshow(cutout, cmap='gray')  # Display as grayscale
        plt.title(f"First Cutout in Batch {batch_idx + 1}")
        plt.show()
        if batch_idx >= 4:  # Display up to 5 batches
            break
